...
describe('[COMP:crm/association-provider] Actual provider object and money admission', () => {
  ...
  it('requires a prior bound object and exact canonical money before any payment effect', async () => {
    ...
    for (const patch of [{ amountMinor: 999 }, { currency: 'EUR' }, { providerReference: 'different' }, { provider: 'other' }]) await expect(f.apply(patch)).rejects.toMatchObject({ code: 'conflict' })
  ...
  it('deduplicates concurrent event ids and semantic duplicates without repeating transitions or notifications', async () => {
    ...
    for (const patch of [{ occurredAt: '2026-09-01T12:00:00.000002Z' }, { metadata: { changed: true } }]) await expect(f.apply(patch)).rejects.toMatchObject({ code: 'conflict' })
  ...
  it('compares every retained legacy evidence field without inventing a historical fingerprint', async () => {
    ...
    await expect(f.apply({ eventId: legacyId, occurredAt: '2026-09-01T12:00:00.000002Z' })).rejects.toMatchObject({ code: 'conflict' })
    await expect(f.apply({ eventId: legacyId, metadata: { changed: true } })).rejects.toMatchObject({ code: 'conflict' })
