...
function ColorField({ label, value, disabled, onValue }: { label: string; value: string; disabled: boolean; onValue(value: string): void }) {
  return <input aria-label={label} disabled={disabled} value={value} placeholder="#RRGGBB" onInput={(event) => { if (colorIsValid(event.currentTarget.value)) onValue(event.currentTarget.value.toUpperCase()); }} onChange={() => undefined} className="h-10 w-24 rounded border bg-background px-2 text-[16px] md:h-7 md:text-xs" />;
}
