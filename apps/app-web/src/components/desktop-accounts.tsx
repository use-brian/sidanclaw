  ...
  return (
    <div className="flex flex-col gap-0.5" aria-busy={pending !== null}>
      {accounts.map((account) => {
        ...
        return (
          <button
            key={account.key} type="button" role="menuitem"
            ...
            className={`flex min-h-11 w-full min-w-0 items-center gap-2 rounded px-2 py-1.5 text-left hover:bg-muted ${account.active ? "bg-muted/60" : ""}`}
          >
            ...
            {pending === account.key ? <span aria-hidden className="size-3 shrink-0 animate-spin rounded-full border-2 border-muted-foreground border-t-transparent" />
