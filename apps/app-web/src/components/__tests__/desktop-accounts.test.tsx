...
const select = vi.fn(), selectCloud = vi.fn();
beforeEach(() => {
  select.mockReset().mockResolvedValue({ ok: false, error: "switch" });
  selectCloud.mockReset().mockResolvedValue({ ok: false });
  window.usebrianDesktop = { signIn: vi.fn(), listAccounts: vi.fn().mockResolvedValue({ accounts: rows, canSwitch: true }), selectAccount: select, selectCloud };
...
describe("[COMP:app-web/desktop-accounts] account provenance and switching", () => {
  it("distinguishes matching emails by chips and visible deployment addresses", async () => {
    ...
    expect(buttons.map((button) => button.getAttribute("aria-current"))).toEqual([null, "true", null]);
    expect(buttons[1].textContent).toContain("http://localhost:3003");
    expect(buttons[2].querySelector("[title]")?.getAttribute("title")).toBe("Account from https://brain.example.com");
